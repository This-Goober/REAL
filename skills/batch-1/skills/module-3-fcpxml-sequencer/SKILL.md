...
This skill is the **last step** of the reel pipeline. Know the pedigree, and
answer it plainly when the director asks "which module made this file":

```
Module 1    (script-architect)  -> report.json         (script timing, media budget)
Module 1.5  (asset-cataloger)   -> asset-catalog.json  (what each file is)
Module 2    (draft-assembler)   -> media.json           (catalog + probe data, reshaped)
                                -> draft.json           (the edit: scenes, layers, timing)
                                -> story.json           (via to_story.py — this skill's input)
Module 3    (this)              -> story.json -> .fcpxml
```

Your input is `story.json` (or `draft.json` + `media.json`, which Module 2's
`to_story.py` turns into one). **You never need the media itself** — an
.fcpxml is a pointer document; the schema carries `asset_duration`, `width`,
`height` precisely so a build can run on a machine the footage isn't on.
Declared metadata works identically to probed; what you lose is verification
(overrun checks are only as good as the declared durations, cover-crop scale
factors are computed from declared dimensions, and rotation flags can't be
confirmed). Say which case you're in before building.
...
## Voice — laying narration when the draft carries none

If real takes exist and the story has no narration lane, laying them is this
module's job (the draft deliberately leaves voice as notes when the clock is
frozen estimated). Place each take as a **connected audio clip** (lane −1,
`role: dialogue`), anchored at the word time where its text begins — never
stretched. The read typically runs a few percent off the estimate, so small
breathing gaps fall between takes; that is correct behavior.

Two rules with scar tissue behind them:

- **A take split by word fraction is a guess.** When a demo splices a take in
  two, derive the split from a measured map first —
  `ffmpeg silencedetect=noise=-30dB:d=0.20` finds sentence boundaries — or
  badge the split as estimated and tell the director which two clips to nudge.
  Measured on this project: the whole-video estimate was 3.8% long but one
  beat was **28%** long — drift is not evenly spread, so a word-fraction
  split can clip a syllable even when the total looks fine.
- **Report drift, never absorb it.** If a section moved far enough that the
  approved pacing no longer holds, say which sections, in the director's
  terms, and offer the re-fit — do not silently re-time an approved edit.

Leave un-placeable pickup fragments out rather than guessing where they
splice, and say so.
...
- **No `/` and no newlines in any name** — project, event, clip, title, gap.
  Final Cut refuses the entire import ("You may not use '/' or the return key
  in names") and the error fires *before* media resolution, so it masquerades
  as a corrupt file. `fcpxml.py` now sanitizes every name centrally in
  `El.set()` (`/` → `·`, newlines stripped); if you ever see that error again,
  the file was built by an old copy of `fcpxml.py`.
- **The desktop bridge belongs to the machine the session started on** — it
  does not follow the director across devices. If Final Cut is on the iMac,
  start the session on the iMac; otherwise work from attached JSONs and
  declared metadata.
- **In-points are offsets into the file actually bound.** If the draft was
  built on a trimmed excerpt, the excerpt file must exist at its path in the
  assets folder — pointing the same in-points at the full-length original
  plays the wrong section (the opening seconds, not the chosen ones).
- **Check masters, not degraded copies.** A conveniently-located 480×640
  export can shadow a 2192×2928 original (cloud aliases hide real filenames:
  `perfect intervals.mov` = `video-851_singular_display.MOV`). Probe or check
  declared dimensions before baking a path.
- **U+202F filenames need no renaming for Final Cut** — the verbatim character
  written into a `file://` URL (`%E2%80%AF`) resolves. Only *staging* chokes
  on it. Don't make the director rename files for an import that would work.
- **Rotation flags (`rot-90`/`rot-180`) are verify-at-import items** — assume
  honored, say so in the hand-off, and offer to bake the rotation in if
  footage comes in sideways.

## When Final Cut rejects an import

Work down `references/fcpxml-notes.md` — **six** things break imports.
...
timebase, a dangling `ref`, a bad `file://` path, and a `/` or newline in any
name (rejected before media resolution — see Rules of the house; the
sanitizer in `fcpxml.py` makes this one impossible from a fresh build).
