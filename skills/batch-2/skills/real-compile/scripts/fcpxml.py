...
INTERNAL. `story.json` is not authored by anyone: it is the intermediate that
`scripts/adapt.py` writes from `reel.json`. This is the `fcpxml` export target,
reached through `scripts/compile.py --target fcpxml`; it is also runnable
directly, which is what you want when debugging a build.
    ...
    def set(self, **attrs):
        for k, v in attrs.items():
            ...
            if key == "name":
                ...
                # return key in names") — and it fires BEFORE media resolution,
                # so it masquerades as a corrupt file. Sanitize centrally so no
                # reel.json can produce an unimportable file. (Ported from the
                # module-3 fix, 2026-08-27.)
...
class Story:
    ...
    def _make_item(self, spec, offset_f, dur_f, lane=None,
        ...
        if kind == "title":
            ...
            # srcEnable="audio" takes just the sound of a movie — a demo
            # clip's own audio under a narration lane, without a second
            # (invisible) copy of its picture riding along.
            if spec.get("src_enable"):
                attrs["srcEnable"] = spec["src_enable"]
        ...
        # --- editorial metadata ---
        # `note` lands in FCP's Notes column; `markers` land in the Timeline
        # Index, which is where a fact like "the beating must be unmistakable
        # by ~2s in" is still useful three revisions later. Both are additive:
        # nothing else in the file depends on them, so if an import ever
        # objects, strip them (adapt.py --no-markers) and everything else
        # still builds.
        if spec.get("note"):
            el.add(El("note")).children.append(str(spec["note"]))
        for mk in spec.get("markers") or []:
            m_f = start_f + self.clock.frames(mk.get("t", 0))
            d_f = max(1, self.clock.frames(mk.get("duration", 0)))
            el.add(El("marker", start=self.clock.t(m_f),
                      duration=self.clock.t(d_f),
                      value=str(mk.get("value", "marker"))))
  ...
  "beats": [                          # the primary storyline, in order
    {
      ...
      "name": "B3 — the demo",             # clip name in FCP
      "note": "narration: so the two waves...",   # FCP Notes column
      "markers": [{"t": 0, "value": "beating unmistakable by ~2s"}],
      "volume_db": -6,
