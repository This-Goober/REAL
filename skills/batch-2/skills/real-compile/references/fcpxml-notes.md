...
**A connected item may not be a `<gap>`.** Read the anchored-item content model
quoted above: `(audio | video | clip | title | caption | mc-clip | ref-clip |
sync-clip | asset-clip | audition | spine | live-drawing)*` — no `gap`, no
`transition`. Gaps are spine-only. So *silence on a lane has to be real media*:
for an unvoiced narration line `adapt.py` writes one silent WAV and cuts each
window out of it, rather than anchoring a gap, which would be a hard rejection
that neither `validate.py` nor `xmllint` would catch without the DTD.
...
Two more were added when this became `/real-compile` and have **not yet been
through a real FCP 11 import**:

- `<note>` and `<marker>` children, which carry the beat's narration, `moment`
  and flags into the Notes column and the Timeline Index. Both are in the DTD's
  content models and `CHILD_RANK` places them correctly (note after
  `text-style-def`, markers last). If an import objects, `adapt.py --no-markers`
  removes every one of them and nothing else changes.
- **`srcEnable="audio"` on an `<asset-clip>`**, which takes only the sound of a
  movie — a demo clip's own audio on its own lane, with no invisible second
  copy of its picture. It is a documented `asset-clip` attribute. If it ever
  causes trouble, drop the attribute: the clip still works, the picture just
  rides along on a negative lane where nothing can see it.
- **a `<gap>` as the first spine item**, which happens when a reel's first beat
  starts after 0. Connected clips anchor to the first spine item, so the
  narration then hangs off a gap whose `start` is `3600s`. That is legal — the
  gap content model allows anchored children — but it has not been seen in a
  real import. If it misbehaves, the local fix is to anchor the globals to the
  first non-gap item in `Story.build()`.
...
## 6. A '/' or newline in any name (ported 2026-08-27)
...
"… (drone / aural suffering / fyi)". Permanent fix: fcpxml.py sanitizes every
name attribute in El.set() ('/' -> '·', newlines stripped). If this error
appears, the file came from an old generator.
