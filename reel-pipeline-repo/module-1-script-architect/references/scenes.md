...
(Vocabulary from the project's editing-principles guide, if it keeps one — use
the director's words, not synonyms.)

A scene is the unit the director vetoes independently. Not every frame; not a
whole paragraph. If they would never say "change just that," it is not its
own scene.
...
1. **Explicit cues in the script.** A `[img: …]` or `[text: …]` bracket is the
   director already deciding. Honour it.
...
the `visual` field — it is what the director reads.
